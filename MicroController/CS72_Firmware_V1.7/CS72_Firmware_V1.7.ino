...
#define FEED_CURRENT 1000 //mA - 1100 is default 1amp. 1100=1.1amp, 900=.9amp, etc
#define SORT_CURRENT 1000 //mA - 1000 is default 1.1 amp.



//STEPPER MOTOR STALL GUARD

#define FEED_DIAG_PIN 13   // FEED DIAG -> D13
#define SORT_DIAG_PIN A1   // SORT DIAG -> A1

// Enable/disable StallGuard handling in firmware
#define STALLGUARD_ENABLED false

#define FEED_STALLGUARD_SENSITIVITY  10   // [0..255] lower = least sensitive / higher number triggers triggers easier
#define SORT_STALLGUARD_SENSITIVITY  25  // [0..255]
...
uint8_t feedSgthrs = FEED_STALLGUARD_SENSITIVITY;
uint8_t sortSgthrs = SORT_STALLGUARD_SENSITIVITY;

volatile bool feedStallLatched = false;
volatile bool sortStallLatched = false;

long feedStepCounter = 0;
long sortStepCounter = 0;
...
void setup() {
  ...
  pinMode(FEED_DIAG_PIN, INPUT_PULLUP);
  pinMode(SORT_DIAG_PIN, INPUT_PULLUP);
  ...
  //StallGuard pin config

  
  // Configure StallGuard in drivers
  if (STALLGUARD_ENABLED) {
    configureStallGuardDriver(feedmotorUART, feedSgthrs);
    configureStallGuardDriver(sortmotorUART, sortSgthrs);
  }
...
void checkSerial(){
  ...
  if(FeedCycleInProgress==false && SortInProgress==false && Serial.available()>0){
       sortStallLatched = false;
       feedStallLatched = false;
       sortStepCounter = 0;
       feedStepCounter = 0;
...
void runSortMotor(){
  if(IsSorting==true){
    ...
    setAccSortDelay();
     // StallGuard check
    if (checkStallGuardWhileMoving(SORT_DIAG_PIN, sortStepCounter)) {
      triggerSortStall(F("error:sort stallguard"));
      //Serial.println(sortmotorUART.SG_RESULT());
      return;
    }
    if(sortStepsToNextPosition > 0){
      stepSortMotor(true);
...
void runFeedMotor() {
  ...
  if(IsFeeding==true && FeedSteps > 0 )
  {
    ...
    if (checkStallGuardWhileMoving(FEED_DIAG_PIN, feedStepCounter)) {
      triggerFeedStall(F("error:feed stallguard"));
      return;
    }
...
void homeFeedMotor(){
  ...
  if(IsFeedHoming==true ){
    ...
    if (checkStallGuardWhileMoving(FEED_DIAG_PIN, feedStepCounter)) {
     ...
     // Serial.println(feedmotorUART.SG_RESULT());
      return;
    }
    stepFeedMotor();
    FeedSteps--;
    ...
    if(IsSorting==true){
         if(digitalRead(SORT_HOMING_SENSOR)!=SORT_HOMING_SENSOR_TYPE){ //
          if(homingSteps < (200*SORT_MICROSTEPS)){
            if (checkStallGuardWhileMoving(SORT_DIAG_PIN, sortStepCounter)) {
             triggerSortStall(F("error:sort stallguard (homing)"));
             //Serial.println(sortmotorUART.SG_RESULT());
            return;
            }
              stepSortMotor(true);  
   ...
   //If sort homing offset true, means we are in offset steps
    if(IsSortHomingOffset == true){
      ...
      if(SortHomingOffsetSteps > 0){
        if (checkStallGuardWhileMoving(SORT_DIAG_PIN, sortStepCounter)) {
          triggerSortStall(F("error:sort offset stallguard (homing)"));
          //Serial.println(sortmotorUART.SG_RESULT());
          return;
          }
        stepSortMotor(true);
...
void configureStallGuardDriver(TMC2209Stepper &drv, uint8_t sgthrs) {
  drv.diag();           // allowed, but not relied upon
  drv.SGTHRS(sgthrs);
  drv.TCOOLTHRS(0xFFFFF);
}

bool diagActive(int pin) {
  // DIAG can be pulsed. With INPUT_PULLUP, “active” is usually LOW if open-drain,
  // but boards vary. Start with LOW-active. If you see constant-triggering, invert it.
  return digitalRead(pin) == LOW;
}

bool checkStallGuardWhileMoving(
   int diagPin,
  long &stepCounter)
{
  ...
  if (!STALLGUARD_ENABLED) return false;
  if (diagPin < 0) return false;

  stepCounter++;
  if (stepCounter < 100) {   // ~first 20 calls = ignore DIAG
    return false;
  }
...
void triggerFeedStall(const __FlashStringHelper* msg) {
  if (feedStallLatched) return;
  feedStallLatched = true;

  Serial.print(F("STALL FEED SG_RESULT="));
  Serial.println(feedmotorUART.SG_RESULT());
  Serial.print(F(", DIAG="));
  Serial.println(digitalRead(FEED_DIAG_PIN));
...
void triggerSortStall(const __FlashStringHelper* msg) {
  if (sortStallLatched) return;
  sortStallLatched = true;

  Serial.print(F("STALL SORT SG_RESULT="));
  Serial.print(sortmotorUART.SG_RESULT());
  Serial.print(F(", DIAG="));
  Serial.println(digitalRead(SORT_DIAG_PIN));
  IsSorting = false;
  SortInProgress = false;
  SortComplete = true;
  IsSortHoming = false;
